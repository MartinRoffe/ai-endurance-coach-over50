...
(`~/.ai_endurance_coach_over50/history.db`) — workout analyses, workout descriptions,
...
The app stores a Garmin session token under `~/.ai_endurance_coach_over50/` and reuses it.
...
2. Remove the stored token files in `~/.ai_endurance_coach_over50/` so the app does a
   fresh login.
3. If you use the scheduled setup, remember the background runs read from
   `~/.ai_endurance_coach_over50/.env`, not your shell — update that copy too.
