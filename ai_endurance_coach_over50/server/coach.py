...
def _coach_system() -> str:
    base = (
        ...
        "HRV traffic light, today's session, and post-workout debrief live there. The Morning Gate "
        "(locked) section is that official call — do NOT rewrite or contradict it unprompted, and do "
        "NOT repeat today's gate advice or post-debrief unless they explicitly ask. Do NOT re-summarise "
        "the latest activity analysis — point them to the Analysis tab or call get_activity_analysis. "
        "For vague questions, clarify scope first (today vs next two weeks). Your default lane is "
        "multi-day plan swaps, event pacing, fuelling depth, and programme philosophy — not "
        "'should I train today?'.\n\n"
        "Live vs morning Body Battery: context has Body battery (AM) and Body battery (now). AM is the "
        "wake/readiness value used by the locked morning gate; (now) is the latest synced reading and "
        "drains through the day with stress and activity. When the athlete asks what their body battery "
        "is, prefer (now) and mention (AM) for context. When they ask about evening / late-day training, "
        "use (now) vs (AM), steps/stress, and session intent — evening BB alone is not a hard stop "
        "(single-digit evening values are common after travel or a stressful day). Prioritise HRV "
        "traffic light, how they feel, and session quality (easy Z2/recovery vs hard intervals).\n\n"
    )
