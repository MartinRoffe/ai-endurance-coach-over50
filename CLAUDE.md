...
**`/position` derives its schedule (2026-07).** The page used to hardcode a Mon/Thu week table, which was wrong in every block — strength is Tue+Thu in the charity plan and position bridge, Thu+Sat through HR Base (Sat = 60 min `Gym — Strength`), Sat-only in the Base deloads, one Wed-or-Thu session from HR wk 14, and absent in both Tenerife camps. `_position_context()` in `server/context.py` now builds today's verdict, the Mon–Sun week, the next 5 sessions and a gap callout from the existing resolvers (`session_for_date_extended` → `hr_session_for_date`, then `kb_spec_for_date` / `_hr_strength_spec`). **A non-None KB spec is the definition of a strength day** — `_strength_on(d)` is the single predicate, so the page cannot drift from the calendars. `_session_contains(spec)` derives "20-min video + single-leg block / + carries" from the exercise id groups (`L*`/`C*`) rather than inventing "Strength 1 / 2" names, which only exist in the bridge and HR plan. Mobility is placed, not scheduled: `_mark_mobility()` picks up to 3 non-strength, non-key-ride days (`_POSITION_KEY_TYPES`), ranking rest days first and all-day camp/charity rides last. `position.html` opens with today → this week → coming up → the how-to blocks, with the essay content (test results, video verdicts, phases, bells, retests, red flags) in collapsed `<details class="ref">`. Tests: `tests/test_position_context.py` pins one week per block, including that `_destack_quality()`'s Wed/Thu swap is respected.
